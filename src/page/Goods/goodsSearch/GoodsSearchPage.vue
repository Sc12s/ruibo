        ...
        <ul class="search_result_box">
            <li v-for="item in searchResult">
                ...
                <div class="goods_probly_info">
                    <p class="goods_price_box">¥{{ item.goods_price }}<span>包邮</span></p>
                    <p class="goods_title_box">{{ item.goods_title }}</p>
                    <p class="goods_merchants_box"><span>
                            <aliwangwang-outlined />
                        </span>{{ item.goods_merchants }}</p>
                </div>
            </li>
        </ul>
    </div>
...
// 搜索请求
const searchGoodsHttp = async () => {
    const { data } = await SEARCH_GOODS({ params: { searchType: searchTypeRef.value, searchInfo: searchInfoRef.value } })
...
const searchGoods = () => { // searchType: string, searchInfo: string
