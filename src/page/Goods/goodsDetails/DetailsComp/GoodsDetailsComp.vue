  ...
  <ul class="goods_box">
    ...
    <li class="goods_parameter">
      ...
      <ul class="goods_size">
        <span>分类：</span>
        <li
          v-for="(size, index) in JSON.parse(details.goods_size)"
          :key="index"
          @click="selectSize(index, size)"
          :class="selectDefault === index && 'selectAfterSize'"
        >
          {{ size }}
        </li>
      </ul>
      ...
      <div class="goods_tags">
        ...
        <a-tag
          color="#f50"
          v-for="(tag, index) in JSON.parse(details.goods_tags)"
          :key="index"
          >{{ tag }}</a-tag
        >
      </div>
      ...
      <div class="goods_nums">
        ...
        <a-input-number
          v-model:value="buy_goods_info.goods_nums"
          :min="1"
...
// 获取商品图片
const httpImageArr = details.value.goods_image_arr;
const imageArr = ref<string[]>(JSON.parse(httpImageArr));

const getImgUrl = (i: number) => {
  return imageArr.value[i];
};
